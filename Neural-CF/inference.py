...
class RecommenderSystem:
    ...
    def eval4sample(self, rating, predict_data):
        ...
        user_business_dict = sample_test.groupby('user_id')['business_id'].agg(lambda x: list(set(x))).to_dict()        
        ...
        hit = 0
        count = 0
        for desired_user_name in tqdm(unique_user_id, desc="Processing Users", unit="user"):
            # if count == 30:
            #     break
            ...
            if is_present:
                ...
                converted_userId = userId_conversion[userId_conversion['user_id'] == desired_user_name]['userId'].values[0]
...
def parse_args():
    ...
    parser.add_argument("--model", type=str, default='1k', help="Choose between model trained on 5k user or 1k user")
    parser.add_argument("--run_eval", action='store_true', help="Run evaluation on the sample test set")
...
if __name__ == "__main__":
    ...
    recommender = RecommenderSystem(args.data_dir, args.model_checkpoint, 
                                    args.userId, args.topK, args.model)
    ...
    if not args.run_eval:
        ...
        print('-'*10 + f'Running Evaluation with Top {args.topK} Prediction' + '-'*10)
