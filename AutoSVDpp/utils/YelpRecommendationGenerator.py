...
class YelpRecommendationGenerator():
    ...
    def generate_sample_user(self):
        ...
        # sample_user_idx = 76
        sample_df = self.df_review_test.loc[self.df_review_test["user_idx"] == sample_user_idx]
        user_df = sample_df.groupby(['user_idx', 'business_idx']).agg({'stars': 'mean'}).reset_index()

        # print the sampled user's profile
        user_id = self.df_review_test[self.df_review_test['user_idx'] == sample_user_idx]['user_id'].iloc[0]
        user_row = self.df_user[self.df_user['user_id'] == user_id]
        print("\n", "=============== generate sample user ", user_id, " ===============", "\n")
        print(user_row)
    ...
    def predict_stars_for_sample_user(self, user_id):
        ...
        df = pd.DataFrame(list(index_to_business_id.items()), columns=['business_idx', 'business_id'])
    ...
    def generate_top_k_recommendations(self, user_id, business_ids=None, k=10):
        ...
        stars_df = sorted_df[:k]
        
        return stars_df, recommendations_df
