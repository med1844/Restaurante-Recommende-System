...
def gen_subset(user_data: List[Json], business_data: List[Json], review_data: List[Json], user_n: int, business_n: int, review_n: int) -> Tuple[List[Json], List[Json], List[Json]]:
    ...
    subset_business_id = {business_id for business_id, _ in subset_business_id_cnt[:business_n]}
    ...
    subset_review_data = sample(subset_review_data, min(review_n, len(subset_review_data)))
